...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        if (_input.nextIf(",")) {
            ...
            result = table1.select(table2, columnTitles, conditions);
        } else {
            ArrayList<Condition> conditions = conditionClause(table1);
            result = table1.select(columnTitles, conditions);
        }
        return result;
    }
    ...
    Condition condition(Table... tables) {
        ...
        if (_input.nextIs(IDENTIFIER)) {
            Column col2 = new Column(columnName(), tables);
            result = new Condition(col1, relation, col2);
        } else if (_input.nextIs(LITERAL)) {
            String val2 = literal();
            result = new Condition(col1, relation, val2);
        } else {
            throw error("invalid second operand");
        }
        return result;
    }
